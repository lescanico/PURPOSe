...
### NLP-based Diagnosis and Medication Classification (Python via reticulate)

```{r nlp_classification, eval=FALSE}
# Step 2.5: Use Python NLP to classify diagnoses and medications
# - Calls a Python script via reticulate to add logical columns for DSM-5 classes and psychotropic groups
# - Saves updated visits_clean for downstream feature engineering
...
# Tell reticulate to use your Python 3.13 installation
reticulate::use_python("C:/Users/lescanon/AppData/Local/Programs/Python/Python313/python.exe", required = TRUE)

# Load cleaned data
visits_clean <- readRDS("data/visits_clean.rds")

# Convert to data.frame for reticulate compatibility
visits_clean_df <- as.data.frame(visits_clean)

# Source the Python script (assumes classify_nlp.py exists in project root)
# The script should define a function: classify_visits(df) -> returns df with new logical columns
source_python("scripts/classify_nlp.py")

# Call the Python function
enriched_visits <- classify_visits(visits_clean_df)

# Convert back to tibble (if needed)
library(tibble)
visits_clean <- as_tibble(enriched_visits)

# Save the updated data for downstream use
saveRDS(visits_clean, "data/visits_clean.rds")
```
...
features <- engineer_utilization(
  ...
  dynamic_vars = c("payer", "pay_net", "portal_engagement", "is_portal_active", "is_tele"),
  ...
  output_path = "data/model_90d_12m.rds",
  filter_var = NULL,
  filter_levels = NULL,
...
if (!file.exists("data/model_90d_12m_patients.rds")) {
  stop("File 'data/model_90d_12m_patients.rds' not found. Run feature engineering first.")
}

patients_feat <- readRDS("data/model_90d_12m_patients.rds")
